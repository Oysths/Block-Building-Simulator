...
int main() {
    ...
    gameHandler.confirmMaps(); //confirms that there are maps in the maps-folder
    //for (int i = -30; i < 30; i++) {
    //    for (int j = 0; j < 1; j++) {
    //        for (int k = -30; k < 30; k++) {
    ...
    while (!window.should_close()) {
        ...
        gameHandler.update(); //basically everything is abstracted to this object, so I can just call this object every frame
